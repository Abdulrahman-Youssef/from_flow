...
class _AddEditDialogState extends State<AddEditDialog> {
  ...
  void _handleSave() {
    ...
    if (_actualArriveDate!.isBefore(now)) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Arrival date cannot be in the past'),
          backgroundColor: Colors.red,
        ),
      );
      return;
    ...
    if (_actualLeaveDate!.isBefore(now)) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Leave date cannot be in the past'),
          backgroundColor: Colors.red,
        ),
      );
      return;
    ...
    if (_actualLeaveDate!.isBefore(_actualArriveDate!) || 
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Leave date must be after arrival date'),
          backgroundColor: Colors.red,
        ),
      );
      return;
  ...
  void _performSave() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Record ${widget.mode == DialogMode.add ? 'added' : 'updated'} successfully'),
